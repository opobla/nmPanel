...
class NmdbOriginalGroupController extends AbstractActionController
{
	protected $model;
	public function __construct($model){
		$this->model= $model;
		date_default_timezone_set("UTC");}
    	public function nmdbOriginalGroupAction(){
		...
		if($start=='all' || $finish=='all'){
			$data=$this->model->originalGroupedAll($points);
			...
			$data=$this->model->originalGroupedInterval($start,$finish,$interval);
		}
		$dataHs=array();
		foreach ($data as $row){
				$dataHs[0][]=array(
